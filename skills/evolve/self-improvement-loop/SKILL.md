...
1. **Fix what may evolve — and what may never.** Classify every surface into four tiers: **editable** (skill bodies, prompt *procedure* sections, memory contents, few-shot examples), **locked** (the eval gate, its rubrics and scoring code — the loop proposes against them, never modifies them), **append-only** (results logs, rejected-proposal records — history the loop reads but can't rewrite), and **human-controlled** (merges, credentials, permission changes, destructive operations). The outside-the-loop invariant: evaluator, budget enforcement, and permission control live *outside* every surface the loop can touch — historically, reward hacks happen precisely where this boundary was violated. And enforce in the **runtime**, not in prompt text: a constraint stated in a mutable prompt is a constraint the loop will eventually evolve away; file permissions, protected paths, and review requirements hold because the loop cannot edit them, not because it was asked nicely.
...
6. **Monitor for loop pathologies** and stop conditions: eval-score plateau with rising change count (thrash), prompt/skill length growing monotonically (accretion — require the loop to also propose deletions), score gain on the harness with human-satisfaction drop (gate gaming — refresh the eval set), same file changed 3 iterations running (oscillation — freeze and escalate). Track **edit diffs, not only scores**: a broken improver that returns its input unchanged produces flat metrics indistinguishable from healthy convergence — alarm on trivial/near-duplicate diffs, and reject near-duplicate proposals before spending eval budget on them. Any pathology pauses the loop and pings the owner.
