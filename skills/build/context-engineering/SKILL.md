...
4. **Compact, don't just truncate.** When the compactible tier exceeds budget, replace old turns with a **structured summary** (decisions made, state established, open threads, artifacts produced) rather than dropping them blind. Truncation loses the fact that mattered on turn 3; a good summary keeps it in a fraction of the tokens. **Crucial:** Never drop task-critical parameters, schema definitions, or active constraints during compaction — over-pruning instructions is the #1 cause of regression when this skill is applied. The **artifact trail** needs verbatim preservation: full file paths, function/variable names, error codes — "modified the config" is useless when the agent needs to find `retry_limit` in `services/api/config.yaml` twenty turns later. Code survives verbatim or referenced by path; only prose gets paraphrased. And treat the summarizer as untrusted: summaries hallucinate plausible details that were never in the original — which then persist as context poisoning (`context-degradation`). Compact on a cadence (every N turns or M tokens), not only at the cliff. Signal-density test for what stays: if removing an element wouldn't change the model's output, it goes.
...
8. **Verify on a long run, against numeric targets — measured per task, not per request.** The metric that matters is **tokens-per-task**: a compaction that saves 0.5% per request but forces the agent to re-fetch what it forgot costs more overall than it saves. Drive the agent through a run long enough to trigger compaction 2–3 times on a task with a fact planted early that's needed late. Confirm: the late step still has the early fact (probe with targeted questions — lexical similarity of the summary proves nothing), quality doesn't step down after each compaction, token-per-turn flattens, and re-fetch/re-read counts don't climb after compactions. Acceptance bars to aim for (measure, don't assume): compaction should reclaim ~50–70% of the compacted span's tokens at under ~5% quality cost on your eval set; masking should reclaim well over half of masked-observation tokens at negligible quality cost. If quality drops more than that, the compaction is over-pruning — see the step-4 warning.
