...
4. **Design the return contract for the caller's use.** Results come back in a shape the coordinator can act on — structured, with status (done / partial / failed / needs-input) and provenance — not a wall of prose the caller must re-parse. Tool-input-style structure beats free text at a handoff boundary (`tool-design` discipline applies). Beware the **telephone game**: every re-summarization hop between agents loses nuance, and after 2-3 hops the original detail is gone. When a worker's finding must reach another worker faithfully, pass the artifact itself (or a direct message/file reference), not the coordinator's summary of a summary. And validate at the boundary: a sub-agent's hallucination returned as a "result" becomes an unquestioned *fact* downstream — checkpoint claims (does the file exist? does the cited source say that?) before they propagate, or one bad hop poisons the whole run.
